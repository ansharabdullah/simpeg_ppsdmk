<div class="widgetbox box">
            <h4 class="widgettitle">RIWAYAT JABATAN</h4>
            <div class="widgetcontent nopadding">
                <form class="stdform stdform2" action="" method="post">
                    <p>
                            <label>Aktif</label>
                            <span class="formwrapper">
                                <input type="checkbox" name="aktif" /><br />
                                </span>
                        </p>
                    <p>
                        <label>Jabatan</label>
                        <span class="field"><input type="text" name="jabatan" class="input-large" placeholder="" /></span>
                    </p>
                    
                    <p>
                        <label>Unit Kerja</label>
                        <span class="field"><input type="text" name="unit_kerja" class="input-large" placeholder="" /></span>
                    </p>

                    <p>
                        <label>No SK</label>
                        <span class="field"><input type="text" name="no_sk" class="input-large" placeholder="" /></span>
                    </p>

                    <div class="par">
                        <label>Tanggal SK</label>
                        <span class="field"><input id="datepicker" type="date" name="tanggal_sk" class="input-small" /></span>
                    </div> 
                   
                    <div class="par">
                        <label>TMT</label>
                        <span class="field"><input id="datepicker" type="text" name="tmt" class="input-small" /></span>
                    </div> 

                    <p>
                        <label>Pejabat</label>
                        <span class="field"><input type="text" name="pejabat" class="input-large" placeholder="" /></span>
                    </p>

                    <p>
                        <label>Angka Kredit</label>
                        <span class="field"><input type="text" name="angka_kredit" class="input-large" placeholder="" /></span>
                    </p>
                    
                    <p>
                        <label>Keterangan</label>
                        <span class="field"><textarea cols="40" rows="3" class="span5" name="keterangan"></textarea></span> 
                    </p>
                    
                    <p class="stdformbutton">
                        <button class="btn btn-primary">Save</button>
                        <button type="reset" class="btn">Cancel</button>
                    </p>
                </form>
            </div>
        </div>   